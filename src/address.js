...
class Address {
    ...
    constructor(addr, config) {
        this.#addr = addr;
        this.#config = config;
        this.#type = addr.substr(0, 2);
    }

    toString() { return this.#addr; }

    describe() {
        const result = {addr: this.#addr};
    ...
    isConfigured() { return this.#config.isConfiguredDevice(this.#addr); }

    isSiteController() { return this.isController() && this.#config.isSiteController(this.#addr); }
